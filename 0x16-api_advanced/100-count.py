...
def hot_print(response, word_list, param, hot_dict):
    ...
    and fill the hot_dict dictionary with that value
    """
...
def count_words(subreddit, word_list, param1=None, hot_dict={}):
    ...
    req = requests.get("{}/r/{}/hot.json".format(url, subreddit),
    ...
    if req:
        next_r = req.json().get("data").get("after")
        if next_r:
            count_words(subreddit, word_list,
                        param1=next_r,
                        hot_dict=hot_dict)

            hot_print(req, word_list, param1, hot_dict)
            return hot_dict
